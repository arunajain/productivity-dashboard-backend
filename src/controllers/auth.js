...
export const verifyEmailCode = async (req, res) => {
  try {
    ...
    if (code_dtl) {
      ...
      await clearVerificationCode(code_dtl.user_id);
      ...
      return res
        .status(200)
        .json({ msg: "The code you entered is incorrect or has expired." });
    }
  } catch (error) {
  ...
  try {
    ...
    if (!user)
      return res.status(401).json({
        success: false,
        msg: "User with given email does not exit",
        user: null,
      });
    ...
    if (!isMatch)
      return res
        .status(400)
        .json({ success: false, msg: "Invalid Password", user: null });
    ...
    return res.status(200).json({
      ...
      user: { id: id, name: name, email: emailId },
      token,
    });
  } catch (error) {
    console.log(error);
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    if (!user)
      res.status(401).json({
        success: false,
        msg: "User with that email does not exit",
        user: null,
      });
...
export const resetPassword = async (req, res) => {
  try {
    ...
    if (code_dtl) {
      ...
      await clearVerificationCode(code_dtl.user_id);
      const hashedPassword = await bcrypt.hash(password, 10);
      await updateUserSingleColumn(
        "password_hash",
        hashedPassword,
        code_dtl.user_id,
      );
      return res.status(200).json({
        msg: "Password Reset successfully. You can now log in to your account with the new password.",
      });
    } else {
      return res
        .status(200)
